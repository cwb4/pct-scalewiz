# -*- coding: utf-8 -*-
"""
Created on Tue Mar 10 08:25:15 2020
...
class ScaleWiz(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.timelimit.set(90), self.failpsi.set(1500)
        self.makestuff()
...
#       plt.tight_layout()
    ...
    def askdir(self):
        ...
        if out == "":
            ...
            pp = p[-2] + " - " + p[-1]
            self.filemenu.entryconfig(1, label=pp)
            self.project.set(pp)
    ...
    def askfil(self, event):
        ...
        event.widget.xview_moveto(1) #BUG for some reason this only fires on second call
        # probably need to do a for child in type method and iterate through all the entry fields each time any one is updates
    ...
    def make_plot(self):
        ...
        if '' in self.pltsrs:
            del self.pltsrs['']
        self.nfig = plt.Figure(figsize=(9.5,5), dpi=100)
        self.nax = self.nfig.add_subplot(111)
...
#        plt.style.use('seaborn-colorblind')
    ...
    def take_reading(self): # loop to be handled by threadpool
        while (self.psi1 < self.failpsi.get()) and (self.psi2 < self.failpsi.get()) and (self.elapsed < self.timelimit.get()*60) and (self.paused == False):
